...
const Login = () => {
    ...
    const onSubmit = (datos) => {
        const verificacionNombre = usuarios.find(usuario => usuario.id === datos.usuario)
        const verificacionEmail = usuarios.find(usuario => usuario.email === datos.email)

        if((verificacionNombre)){
            Swal.fire("Este usuario es existente", "Prueba registrarte con otro nombre de usuario", "error")
            return
        } else if((verificacionEmail)){
            Swal.fire("Este email es existente", "Prueba registrarte con otro email", "error")
            return            
        }
        else {
        setUsuarios([
        ...
        Swal.fire(`Te registraste correctamente, ${usuario}`, "Inicia sesion con tu nueva cuenta.", "success")
    ...
    <div className="row justify-content-center">
        <div className="col-12 col-sm-8 col-md-6 col-xl-4">
            ...
            <Form.Control type="text" 
                ...
                value={email}
                />
                <Form.Text className="text-danger mb-2">{errors.email?.message}</Form.Text>
                </Form.Group>
                <Form.Group className='mb-2'>
                <Form.Control
                type='password' 
